<{$block.HotFooTableJS}>
<table summary="list_table" id="hot_discuss" cellspacing="1" class="table outer table">
    <thead>
      <tr>
  ...
  <{foreach item=discuss from=$block.discuss}>
    <tr class="<{$discuss.class}>">
      ...
      <td headers="discuss_title">
        <img src="<{$xoops_url}>/modules/tad_discuss/images/<{if $discuss.isPublic|default:false}>greenpoint.gif<{else}>lock.png<{/if}>" alt="<{$discuss.DiscussTitle}>" title="<{$discuss.DiscussTitle}>" align="absmiddle" style="margin-right:3px;"><a href="<{$xoops_url}>/modules/tad_discuss/discuss.php?DiscussID=<{$discuss.DiscussID}>&BoardID=<{$discuss.BoardID}>" style="color:#505050"><{$discuss.showDiscussTitle}></a>
      </td>
      ...
      <td headers="discuss_uid">
        ...
        <div><{if $discuss.uid|default:false}><a href="<{$xoops_url}>/userinfo.php?uid=<{$discuss.uid}>"><{$discuss.uid_name}></a><{else}><{$discuss.uid_name}><{/if}></div>
      </td>
      <td headers="discuss_last_re">
        <div style="font-size: 62.5%;"><{$discuss.LastTime}></div>
        <div><{if $discuss.last_uid|default:false}><a href="<{$xoops_url}>/userinfo.php?uid=<{$discuss.last_uid}>"><{$discuss.last_uid_name}></a><{else}><{$discuss.last_uid_name}><{/if}></div>
      </td>
    </tr>
  <{/foreach}>
