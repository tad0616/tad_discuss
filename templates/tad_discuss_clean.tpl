<div style="border:1px solid #cfcfcf;margin:4px;padding:10px;background:<{if $discuss.onlyTo|default:false}>#FEEDD3<{else}>white<{/if}>;-moz-border-radius: 5px;-khtml-border-radius: 5px;-webkit-border-radius: 5px;border-radius: 5px;">
  ...
  <{if $discuss.show_sig and $discuss.user_sig}>
    ...
    <div style="<{if $discuss.sig_style|default:false}><{$discuss.sig_style}><{else}>font-size: 75%; color: gray; border-top: 1px dashed gray; padding: 10px 0px; margin: 10px 0px;<{/if}>">
      <{$discuss.user_sig}>
    </div>
  <{/if}>
</div>