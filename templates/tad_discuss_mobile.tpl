...
<table>
  <tr>
    ...
    <td style="background-color:white;vertical-align:top;">
        ...
        <{if $discuss.DiscussID|default:false}>
          <{if $discuss.onlyTo|default:false}>
            <{if $smarty.session.tad_discuss_adm or $now_uid==$discuss.uid}>
              <a href="discuss.php?op=unlock&BoardID=<{$discuss.BoardID}>&DiscussID=<{$discuss.DiscussID}>&ReDiscussID=<{$ReDiscussID|default:''}>" class="mobile_btn" title="<{$smarty.const._MD_TADDISCUS_LOCK}>"><img src="images/lock.png" alt="<{$smarty.const._MD_TADDISCUS_LOCK}>"></a>
            <{else}>
              <img src="images/lock.png" alt="<{$smarty.const._MD_TADDISCUS_ONLY_ROOT}>">
            <{/if}>
          <{else}>
            <{if $smarty.session.tad_discuss_adm or $now_uid==$discuss.uid}>
              <a href="discuss.php?op=lock&BoardID=<{$discuss.BoardID}>&DiscussID=<{$discuss.DiscussID}>&ReDiscussID=<{$ReDiscussID|default:''}>" class="mobile_btn" title="<{$smarty.const._MD_TADDISCUS_UNLOCK}>"><img src="images/unlock.png" alt="<{$smarty.const._MD_TADDISCUS_UNLOCK}>"></a>
            <{/if}>
          <{/if}>
        <{/if}>
      ...
      <div style="background-color:<{if $discuss.onlyTo|default:false}>#FEEDD3<{else}>#EEEEEF<{/if}>;width:100%;height:76px;margin-top:10px;">
